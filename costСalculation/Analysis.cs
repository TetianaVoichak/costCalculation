...
namespace costСalculation
{
    internal static class Analysis
    {
        ...
        //проверяем введены ли корректно год и месяц / check if the year and month are entered correctly
        ...
        public static List<CategorySum> CalculationForAnalysis(int month, int year,List<int> YEARS, costsOfDay costsOf,List<Category> categories)
        {
            ...
            if (checkYearAndMonth(year, month, YEARS))
            {
                ...
                categorySum = costsOf.ResultListSumMoneyFromMonthCategory(listInfo,categories); //на основе этого списка будет строится диаграмма / a diagram will be built based on this list

            }
            return categorySum;
        }
        //общая сумма по категории / total amount by category
        ...
        //считает процент от суммы для каждой категории / calculates the percentage of the total for each category
        ...
        //делает метку с названием категории и ее процентом от общей суммы /  a label with the category name and its percentage of the total
        ...
        // возращает метку с названием категории и ее процентом от общей суммы / return a label with the category name and its percentage of the total
